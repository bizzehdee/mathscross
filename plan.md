...
/
  ...
  native/
    ...
    package.json
    hooks/
    res/icon/android/         # launcher icons. Generated by npm run icons, committed
...
The same script writes the Android launcher icon into `native/res/icon/android/`,
and `native/config.xml` names those files per density. This is not optional: a
Cordova shell with no `<icon>` elements ships the default Cordova mark, the store
listing carries the generated one, and Play rejects the mismatch — which is how it
was found. Both forms are generated, the adaptive foreground and background layers
Android 8+ composites and the flat icon below it. CI regenerates every icon and
fails on a diff; `release.yml` compares the bytes Cordova copied into the platform's
mipmap resources against the sources, because a bad path in `config.xml` is only a
warning. See `.learnings/installed-icon-must-match-store.md`.
...
| `installed-icon-must-match-store.md` | changing the icon generator, or adding an asset only the packaged app uses |
...
- **M5 — Persistence, daily, PWA. Done.** `game/persist.ts` with the two slots,
  `features/stats`, `features/daily`, `features/theme`, `ui/stats`, `ui/settings`,
  `ui/install/service-worker.ts`, and the icons from section 9.5 generated by
  `scripts/generate-icons.mjs` on top of a dependency-free PNG encoder in
  `scripts/png.mjs`.

  Verified in a browser: a board resumes with its entries, clock and undo history; a
  solved board records one completion and clears its slot; the daily is date-keyed,
  7 x 7 on a Wednesday per the rotation, and stored **without a solution field**; both
  slots coexist; theme choice persists and `system` removes the attribute rather than
  setting it.

  Two defects found by running it. Starting a new puzzle asked to protect work that
  was already finished, because `hasProgress` counted a solved board as in progress.
  And the shipped `connect-src 'none'` blocks Vite's hot-update websocket, so dev
...
| The store icon drifts from the installed icon | The listing and the home screen disagree, and Play rejects the submission | `store/icon-512.png` is copied from `public/icons/`, never redrawn, and the Android launcher icon is generated by the same script and asserted in CI and at release. Sections 9.5 and 9.6 |
